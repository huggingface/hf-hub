//! Built-in [`indicatif`] progress renderer, gated behind the `indicatif` feature.
...
use super::{DownloadEvent, FileProgress, FileStatus, ProgressEvent, ProgressHandler, UploadEvent};

const MAX_VISIBLE_FILE_BARS: usize = 10;
const MAX_VISIBLE_UPLOAD_BARS: usize = 10;

/// Ready-made [`ProgressHandler`] that renders [`indicatif`] progress bars on the
/// terminal for download and upload operations.
///
/// This is the batteries-included alternative to hand-writing a handler: enable the
/// `indicatif` feature and pass an `IndicatifProgress` to any `.progress(...)` setter to
/// get download, snapshot-download, and upload progress bars for free. It renders:
///
/// - a single byte bar for single-file downloads,
/// - a files-completed bar plus per-file byte bars (capped, with a queue) for multi-file snapshot downloads,
/// - an aggregate byte bar for xet batch transfers,
/// - a preparing/committing spinner and processing/transfer bars for uploads.
///
/// Output is drawn to stderr and, thanks to indicatif, is automatically suppressed
/// when stderr is not a terminal, so it is safe to attach unconditionally.
///
/// # Example
///
/// ```no_run
...
/// ```
///
/// To interleave your own log output above the bars without visual corruption, build
/// the handler from a shared [`MultiProgress`] with [`IndicatifProgress::with_multi`]
/// and route your logs through the same `MultiProgress`.
...
impl IndicatifProgress {
    /// Create a handler that renders to its own [`MultiProgress`] targeting stderr.
    pub fn new() -> Self {
        Self::with_multi(MultiProgress::new())
    }

    /// Create a handler that renders through the provided [`MultiProgress`].
    ///
    /// Use this to share a single `MultiProgress` between this handler and other
    /// output (for example an `io::Write` adapter that calls
    /// [`MultiProgress::println`]) so log lines stay above the bars.
    ...
    /// The [`MultiProgress`] this handler renders through.
