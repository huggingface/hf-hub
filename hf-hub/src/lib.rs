...
//! - **Optional progress bars** — a ready-made terminal progress renderer (`IndicatifProgress` in [`progress`]) when
//!   the `indicatif` feature is enabled.
//!
//! ## Creating a client
//!
...
//! - `indicatif` — ships the built-in `IndicatifProgress` terminal progress handler in the [`progress`] module. Off by
//!   default; native only (no effect on wasm).
