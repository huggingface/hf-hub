//! Repository commit and upload builders.
//!
//! Requires the `upload` feature (default-on). Builders on [`HFRepository`] for uploading repo
//! contents. For delete-only commits (which never move file content and stay available without
//! the `upload` feature), see [`crate::repository::delete`] instead.
//!
